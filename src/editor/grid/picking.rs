...
pub fn update_hover(
    ...
    let distance =
        (offset - near.dot(normal)) / denominator;
...
pub fn raycast_world(
    ...
    while distance < RAYCAST_MAX_DISTANCE {
        if let Some(cell) = world.get(map_pos) {
            ...
            if eligible {
                let selectable = world.is_cell_visible(map_pos)
                    || include_markers;
...
fn screen_ray(
    ...
    let near =
        inverse_view_projection.project_point3(
            Vec3::new(
                normalized_x,
                normalized_y,
                -1.0,
...
fn initial_side_distance(
    ...
    if direction < 0.0 {
        ...
        (cell_coordinate as f32 + 1.0 - coordinate)
            * delta_distance
    }
}