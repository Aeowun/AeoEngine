...
fn ranked_asset_matches<I>(names: I, query: &str, limit: usize) -> Vec<String>
where
    I: IntoIterator,
    I::Item: AsRef<str>,
{
...
fn draw_texture_browser(ctx: &egui::Context, selected_texture: &mut String) {
    let id = egui::Id::new(TEXTURE_BROWSER_ID);
...
pub fn draw_tool_bar(ui: &mut Ui, editor: &mut Editor, project_path: &Option<PathBuf>) {
    ...
    draw_texture_browser(ui.ctx(), &mut editor.build_template.texture);

    draw_audio_browser(ui.ctx(), &mut editor.build_template.audio, project_path);
}
