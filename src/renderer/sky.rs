...
impl SkyRenderer {
    pub fn new() -> Self {
        ...
        // Right face (+X)
        add_face(&mut vertices, [0.5, -0.5, -0.5], [0.5, -0.5, 0.5], [0.5, 0.5, 0.5], [0.5, 0.5, -0.5]);
        // Left face (-X)
        add_face(&mut vertices, [-0.5, -0.5, 0.5], [-0.5, -0.5, -0.5], [-0.5, 0.5, -0.5], [-0.5, 0.5, 0.5]);
        // Top face (+Y)
        add_face(&mut vertices, [-0.5, 0.5, 0.5], [0.5, 0.5, 0.5], [0.5, 0.5, -0.5], [-0.5, 0.5, -0.5]);
    ...
    pub fn render(&self, renderer: &Renderer, world: &World, camera_pos: Vec3, view_projection: Mat4) {
        ...
        unsafe {
            ...
            gl::UniformMatrix4fv(vp_loc, 1, gl::FALSE, view_projection.to_cols_array().as_ptr());
            ...
            let tex_loc = gl::GetUniformLocation(self.program, CString::new("u_cubemap").unwrap().as_ptr());
...
fn add_face(vertices: &mut Vec<f32>, v1: [f32; 3], v2: [f32; 3], v3: [f32; 3], v4: [f32; 3]) {
    ...
    // Triangle 1
    vertices.extend_from_slice(&v1); vertices.extend_from_slice(&normal); vertices.extend_from_slice(&color); vertices.extend_from_slice(&dummy_uv);
    vertices.extend_from_slice(&v2); vertices.extend_from_slice(&normal); vertices.extend_from_slice(&color); vertices.extend_from_slice(&dummy_uv);
    vertices.extend_from_slice(&v3); vertices.extend_from_slice(&normal); vertices.extend_from_slice(&color); vertices.extend_from_slice(&dummy_uv);

    // Triangle 2
    vertices.extend_from_slice(&v1); vertices.extend_from_slice(&normal); vertices.extend_from_slice(&color); vertices.extend_from_slice(&dummy_uv);
    vertices.extend_from_slice(&v3); vertices.extend_from_slice(&normal); vertices.extend_from_slice(&color); vertices.extend_from_slice(&dummy_uv);
    vertices.extend_from_slice(&v4); vertices.extend_from_slice(&normal); vertices.extend_from_slice(&color); vertices.extend_from_slice(&dummy_uv);
}
